...
public class TaxiDriversProblem {
	...
	class MyCallableWalker implements Callable<Integer> {
		int i, n, myCounter;
		boolean[] boolArray;

		public MyCallableWalker(int inputI, int inputN) {
			i = inputI;
			n = inputN;
		}
		...
		public Integer call() throws Exception {
			...
			walkPath(i, carh, carv);
			atomicCounter.addAndGet(myCounter);
			return myCounter;
		}

		public void walkPath(int lastJ, long h, long v) {
			...
			for (Integer nextJ : (myLastSet = myJuncArrayMap.get(lastJ)).keySet()) {
				if (!boolArray[nextJ]
						...
						&& (myV = (v - myNextEdge.v)) >= 0) {
					walkPath(nextJ, myH, myV);
				}
			}
		}
	}

	public static void algo11() { // 2 threads 63 seconds, 10 threads 29 seconds 50 threads 29 seconds. my PC is capped.
		TaxiDriversProblem mytd = new TaxiDriversProblem();
		Scanner scanner = new Scanner(System.in);
		int n = scanner.nextInt();
		carh = scanner.nextLong();
		carv = scanner.nextLong();
		...
		// long walkedPath = 0;
		//visitedCounter = 0;
		long possiblePaths = (n * n - n) / 2; // possible paths is n^2, less the
												// path from node to itself,
												// divided by 2 because of
												// unordered pairs
		...
		for (int i = 1; i < n + 1; i++) {
			...
			myJuncArrayMap.add(new HashMap<Integer, Edge>());
			callables.add(mytd.new MyCallableWalker(i,n));
			// System.out.println("10 juncture " + myJunc[i].x + "," +
			// myJunc[i].y);
		}

		for (int i = 0; i < n - 1; i++) { // represent Path as matrix of
											// nondirectional edges between
											// Junctions 1 and 2
		...
		// All junctures are connected to each other. All junctures have exactly
		// 1 shortest path to each other
		try {
			List<Future<Integer>> futures = exServ.invokeAll(callables);
			
			//for (Future<Integer> future: futures) {
				//System.out.println ("future.get = " + future.get());
				//visitedCounter += future.get();
			//}
		} catch (Exception e) {
		...
		System.out.println("10 possiblePaths " + possiblePaths + " walkedPaths " + (atomicCounter.get() - n) / 2
				+ " time taken " + (System.currentTimeMillis() - startTime));
		System.out.println(possiblePaths - (atomicCounter.get() - n) / 2); // walked divided by 2 because of unordered pairs
	...
	public static void algo10() { // faster under 2 minutes under 105 seconds
		...
		for (int i = 1; i < n + 1; i++) { // walk n iterations
			...
			// System.out.println ("10 Walked From " + i + " found paths " +
			// visitedCounter );
		}

		System.out.println("10 possiblePaths " + possiblePaths + " walkedPaths " + (visitedCounter - n) / 2
				+ " time taken " + (System.currentTimeMillis() - startTime));
		System.out.println(possiblePaths - (visitedCounter - n) / 2); // walked
																		// path
																		// divided
																		// by 2
																		// because
	...
	public static void walkPath10(int lastJ, long h, long v, boolean[] boolArray) {
		...
		long myH, myV;
		for (Integer nextJ : (myLastSet = myJuncArrayMap.get(lastJ)).keySet()) {
			if (!boolArray[nextJ] && (myH = (h - (myNextEdge = myLastSet.get(nextJ)).h)) >= 0
					&& (myV = (v - myNextEdge.v)) >= 0) { // can go from last to
															// next, and next
															// not already
															// visited
				// System.out.println(" 10 Found Path from " + lastJ + " to " +
				// nextJ
				// + " remaining h " + h + " v " + v
				// + " next h " + Math.abs(myJunc[lastJ].x - myJunc[nextJ].x) +
				// " v " + Math.abs(myJunc[lastJ].y - myJunc[nextJ].y));
	...
	public static void algo9() { // Trading Space for Time
		...
		// ArrayList<HashSet<Integer>> myJuncArraySet = new
		// ArrayList<HashSet<Integer>>(n+1);
		visitedCounter = 0;
		long possiblePaths = (n * n - n) / 2; // possible paths is n^2, less the
												// path from node to itself,
												// divided by 2 because of
		...
		for (int i = 1; i < n + 1; i++) {
			...
			// myJuncArraySet.add(new HashSet<Integer>());
			myArrayMapEdge.add(new HashMap<Integer, Edge>());
			// System.out.println("9 juncture " + myJunc[i].x + "," +
			// myJunc[i].y);
		}
			...
			while (que.size() > 0) {
				...
				for (Integer nextJ : myArrayMapEdge.get(lastJ).keySet()) {
					if (!visitedArray[nextJ] // hasn't visited from i to nextJ
							// && (visitedH[nextJ] = visitedH[lastJ] +
							// Math.abs(myJunc[lastJ].x - myJunc[nextJ].x)) <=
							// carh // next Step doesn't exceed car's max
							// distance
							// && (visitedV[nextJ] = visitedV[lastJ] +
							// Math.abs(myJunc[lastJ].y - myJunc[nextJ].y)) <=
							// carv ) {
							&& (visitedH[nextJ] = visitedH[lastJ] + myArrayMapEdge.get(lastJ).get(nextJ).h) <= carh // next
																													// Step
																													// doesn't
																													// exceed
																													// car's
						...
						// System.out.println ("9 at " + i + " from " + lastJ +
						// " to " + nextJ);
						que.add(nextJ); // add to queue the set of edges for
										// next iteration
					}
				}
			}

			// walkedPath += visitedCounter - 1; // found all walked paths,
			// except walking to self.
			// System.out.println ("9 Walked From " + i + " found paths " +
			// (visitedCounter));
		}

		System.out.println("9 possiblePaths " + possiblePaths + " visitedCounter " + (visitedCounter - n) / 2
				+ " time taken " + (System.currentTimeMillis() - startTime));
		System.out.println(possiblePaths - (visitedCounter - n) / 2); // walked
																		// path
																		// divided
	...
	public static void algo8() { // Without recursion
		...
		for (int i = 1; i < n + 1; i++) {
			...
			myJunc[i].y = scanner.nextInt();
			myJuncArrayMap.add(new HashMap<Integer, Edge>());
			// System.out.println("8 juncture " + myJunc[i].x + "," +
			// myJunc[i].y);
		}
		...
		for (int i = 1; i < n + 1; i++) { // walk n iterations
			...
			while (que.size() > 0) {
				...
				for (Integer nextJ : myJuncArrayMap.get(lastEdge.junc).keySet()) {
					...
					if (myRemainingH >= 0 && myRemainingV >= 0 && visitedArray[nextJ] == 0) {
						que.add(mytd.new TravelEdge(nextJ, myRemainingH, myRemainingV)); // add
																							// to
																							// queue
																							// the
																							// set
			...
			walkedPath += visitedCounter - 1; // found all walked paths, except
												// walking to self.
			System.out.println("8 Walked From " + i + " found paths " + visited.size() + " total paths " + walkedPath);
		}

		System.out.println("8 possiblePaths " + possiblePaths + " walkedPaths " + walkedPath / 2 + " time taken "
				+ (System.currentTimeMillis() - startTime));
		System.out.println(possiblePaths - (walkedPath / 2)); // walked path
																// divided by 2
																// because of
																// unordered
																// pairs
	...
	public static void algo7() {
		...
		for (int i = 1; i < n + 1; i++) {
			...
			myJunc[i].y = scanner.nextInt();
			myJuncArrayMap.add(new HashMap<Integer, Edge>());
			// System.out.println("7 juncture " + myJunc[i].x + "," +
			// myJunc[i].y);
		}
		...
		for (int i = 1; i < n + 1; i++) { // walk n iterations
			...
			walkedPath += visitedCounter; // found all walked paths, except
											// walking to self.
			// System.out.println ("7 Walked From " + i + " found paths " +
			// visited.size() + " total paths " + walkedPath );
		}

		System.out.println("7 possiblePaths " + possiblePaths + " walkedPaths " + walkedPath / 2 + " time taken "
				+ (System.currentTimeMillis() - startTime));
		System.out.println(possiblePaths - (walkedPath / 2)); // walked path
																// divided by 2
																// because of
	...
	public static void walkPath7(int lastJ, long h, long v) {
		...
		for (Integer nextJ : myJuncArrayMap.get(lastJ).keySet()) {
			...
			if (myX <= h && myY <= v && visitedArray[nextJ] == 0) { // can go
																	// from last
																	// to next,
																	// and next
																	// not
																	// already
																	// visited
				// System.out.println(" 4 Found Path from " + lastJ + " to " +
				// nextJ
				// + " remaining h " + h + " v " + v
				// + " next h " + Math.abs(myJunc[lastJ].x - myJunc[nextJ].x) +
				// " v " + Math.abs(myJunc[lastJ].y - myJunc[nextJ].y));
	...
	public static void algo6() { // fastest at 145 to 165seconds
		...
		for (int i = 1; i < n + 1; i++) {
			...
			myJunc[i].y = scanner.nextInt();
			myJuncArrayMap.add(new HashMap<Integer, Edge>());
			// System.out.println("6 juncture " + myJunc[i].x + "," +
			// myJunc[i].y);
		}
		...
		for (int i = 1; i < n + 1; i++) { // walk n iterations
			...
			walkedPath += visited.size() - 1; // found all walked paths, except
												// walking to self.
			System.out.println("6 Walked From " + i + " found paths " + visited.size() + " total paths " + walkedPath);
		}

		System.out.println("6 possiblePaths " + possiblePaths + " walkedPaths " + walkedPath / 2 + " time taken "
				+ (System.currentTimeMillis() - startTime));
		System.out.println(possiblePaths - walkedPath / 2); // walked path minus
															// divided by 2
															// because of
															// unordered pairs
		// Solution 226330206 for problem 5
	...
	public static void walkPath6(int lastJ, long h, long v) {
		...
		for (Integer nextJ : myJuncArrayMap.get(lastJ).keySet()) {
			int myX = myJuncArrayMap.get(lastJ).get(nextJ).h;
			int myY = myJuncArrayMap.get(lastJ).get(nextJ).v;
			if (myX <= h && myY <= v && !visited.contains(nextJ)) { // can go
																	// from last
																	// to next,
																	// and next
																	// not
	...
	public static void algo5() {
		...
		for (int i = 1; i < n + 1; i++) { // walk n iterations
			...
			walkedPath += visited.size() - 1; // found all walked paths, except
												// walking to self.
			// System.out.println ("5 Walked From " + i + " found paths " +
			// visited.size() + " total paths " + walkedPath );
		}

		System.out.println("5 possiblePaths " + possiblePaths + " walkedPaths " + walkedPath / 2 + " time taken "
				+ (System.currentTimeMillis() - startTime));
		System.out.println(possiblePaths - (walkedPath / 2)); // walked path
																// divided by 2
																// because of
	...
	public static void algo4() {
		...
		for (int i = 1; i < n + 1; i++) {
			...
			// System.out.println("4juncture " + myJunc[i].x + "," +
			// myJunc[i].y);
		}
		...
		for (int i = 1; i < n + 1; i++) { // walk n iterations
			...
			walkedPath += visited.size() - 1; // found all walked paths, except
												// walking to self.
			// System.out.println ("4 Walked From " + i + " found paths " +
			// visited.size() + " total paths " + walkedPath );
		}

		System.out.println("4 possiblePaths " + possiblePaths + " walkedPaths " + walkedPath / 2 + " time taken "
				+ (System.currentTimeMillis() - startTime));
		System.out.println(possiblePaths - (walkedPath / 2)); // walked path
																// divided by 2
																// because of
	...
	public static void algo3() {
		...
		// System.out.println ("3 Possible Paths " + possiblePaths + " total
		// paths can travel " + totalPathsCanTravel);
	...
	public static void walkPaths(ArrayList<Path> myiPaths, int fromJunc, int viaJunc, long h, long v) {
		for (Path myiPath : myiPaths) { // walk every path
			if (h >= myiPath.h && v >= myiPath.v) { // if you can still get
													// there
				int toJunc;
				if (viaJunc == myiPath.junc1)
					toJunc = myiPath.junc2;
				else
					toJunc = myiPath.junc1;
				if (fromJunc == toJunc) {
					System.out.println("Detected loop from " + fromJunc);// loop
																			// detection
					continue;
				}
				if (myTravelledPath.get(fromJunc).get(toJunc) == null) { // mark
																			// this
																			// path
																			// has
																			// been
																			// counted
					// System.out.println("3 from "+ fromJunc + " to " + toJunc
					// + " can travel via " + viaJunc );
					// totalPathsCanTravel++;
	...
	public static void algo2() {
		...
		long possiblePaths = n * n - n; // possible paths is n^2, less the path
										// from node to itself.
		long pathTooLong = 0;
		...
		for (int i = 0; i < n - 1; i++) { // represent Path as matrix of
			...
			myPath[u][v].hops = 1;
			myPath[v][u] = myPath[u][v]; // initialize both sides of the
											// diagonal to make discovery
											// easier.
			System.out.println("2Path loaded " + possiblePaths + " " + u + " to " + v + " h " + myPath[u][v].h + " v "
					+ myPath[u][v].v);
			...
			for (int j = 1; j < i; j++) { // walk the 2nd dimension
				...
				for (int k = 1; k < n + 1; k++) {
					...
					if (myPath[i][k] == null) { // found new Path!
						...
						myPath[k][i] = myPath[i][k]; // initialize both sides of
														// the diagonal to make
														// discovery easier.
						System.out.println("2Path found " + possiblePaths + " " + i + " to " + k + " h "
								+ myPath[i][k].h + " v " + myPath[i][k].v + " hops " + myPath[i][k].hops);
					} else if (myPath[i][k] != null // previous path found
							&& myPath[i][k].hops > myPath[i][j].hops + myPath[j][k].hops) {// found
																							// shorter
																							// connection
						myPath[i][k].h = myPath[i][j].h + myPath[j][k].h;
						myPath[i][k].v = myPath[i][j].v + myPath[j][k].v;
						myPath[i][k].hops = myPath[i][j].hops + myPath[j][k].hops;
						System.out.println("2Shorter Path found " + possiblePaths + " " + i + " to " + k + " h "
								+ myPath[i][k].h + " v " + myPath[i][k].v + " hops " + myPath[i][k].hops);
					}
				}
			}
	...
	public static void algo1() { // brute force
		...
		for (int i = 0; i < n - 1; i++) {
			...
			myPath[v][u] = myPath[u][v]; // initialize both sides of the
											...
											// easier.
			possiblePaths--; // count
			System.out.println("Path loaded " + possiblePaths + " " + u + " to " + v + " h " + myPath[u][v].h + " v "
					+ myPath[u][v].v);
		...
		while (possiblePaths > 0) { // loop until all paths found
			for (int i = 1; i < n + 1; i++) {
				for (int j = 1; j < n + 1; j++) {
					for (int k = 1; k < n + 1; k++) {
						if (i != j && j != k && i != k && myPath[i][j] != null && myPath[j][k] != null
							...
							myPath[i][k].v = myPath[i][j].v + myPath[j][k].v;
							myPath[i][k].hops = myPath[i][j].hops + myPath[j][k].hops;
							myPath[k][i] = myPath[i][k]; // initialize both
															// sides of the
															// diagonal to make
															// discovery easier.
							possiblePaths--; // count
							System.out.println("Path found " + possiblePaths + " " + i + " to " + k + " h "
									+ myPath[i][k].h + " v " + myPath[i][k].v + " hops " + myPath[i][k].hops);
						} else if (i != j && j != k && i != k && myPath[i][j] != null && myPath[j][k] != null
								&& myPath[i][k] != null && myPath[i][k].hops > myPath[i][j].hops + myPath[j][k].hops) { // found
																														// shorter
																														// connection
							myPath[i][k].h = myPath[i][j].h + myPath[j][k].h;
							myPath[i][k].v = myPath[i][j].v + myPath[j][k].v;
							...
							myPath[k][i] = myPath[i][k]; // initialize both
															...
															// diagonal to make
															// discovery easier.
							System.out.println("Shorter Path found " + possiblePaths + " " + i + " to " + k + " h "
									+ myPath[i][k].h + " v " + myPath[i][k].v + " hops " + myPath[i][k].hops);
						}
					}
				}
