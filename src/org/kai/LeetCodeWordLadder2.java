...
public class LeetCodeWordLadder2 {



public static void main(String[] args) {
	// https://leetcode.com/problems/word-ladder/
	/* Given two words (beginWord and endWord), and a list of words, find length of shortest transformation sequence
	...
	public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {  // time limit exceeded.  
		...
		dfs(beginWord,endWord, rv, currentList, length);
		return rv;
	}

	public void dfs(String beginWord, String endWord, List<List<String>> rv, List<String> currentList,  int length) {
		// end conditions. I am exactly 1 short of the target length
		if (currentList.size() == length-1) {
			//System.out.println ("currentList " + currentList);
			if (m.get(beginWord).contains(endWord)) {  // and target word is my neighbor.  success condition
				List<String> addMe = new ArrayList<String>(currentList);
				addMe.add(endWord);
				rv.add(addMe);  // clone this list because it is good
			}
				return;  // success or failure, return
		}
		
		// so I haven't hit my target length yet.  DFS with backtracking to hit all permutations
		for (String w: m.get(beginWord)) {  // iterate through all neighbors not yet visited.
			if (!currentList.contains(w)) { // walked to neighbors not yet visited
				currentList.add(w);
				//System.out.println("   currentList " + currentList );
				dfs(w, endWord, rv, currentList,  length); // walk from endWord to beginWord
	...
	public int ladderLength(String beginWord, String endWord, List<String> wordList) {
		/* Strategy
		 * Define function for neighbors
		 * Create Map<String,Set<String>> which maps every word and its neighbors
		 * BFS
		 */
		if (!wordList.contains(endWord))  // end word must be in wordList otherwise no path
			return 0;
		
		m = new HashMap<>();
		m.put(beginWord, new HashSet<>());


		 // Create Map<String,Set<String>> which maps every word and its neighbors
		for (String w: wordList) {
			m.put(w, new HashSet<>());
			// look in Map's keySet for neighbors.  If found, add neighbor to my set, and and me to my neighbor's set.  Bidirectional graph
			for (String w2: m.keySet()) {
				if (isNeighbor(w, w2)) {
					m.get(w).add(w2);
					m.get(w2).add(w);
				}
	...
	// BFS -- iterative
	public int bfs(String beginWord, String endWord, Map<String, Set<String>> m) {
		Set<String> visited = new HashSet<>();
		Deque<String> q = new ArrayDeque<>();
		int currentCount=1, childCount=0, depth=0;
		q.add(endWord);
		...
		while (!q.isEmpty()) {
			...
			if (word.equals(beginWord))  // success condition
				return depth+1;

			visited.add(word);  // register my visit

			// add the next generation to be visited 
			Set<String> s = m.get(word);
			for (String w: s) {
		...
		// if reached here without returning success condition, then fail condition
		return 0;	
	}
	
	// Define function for neighbors
	public boolean isNeighbor (String w1, String w2) {
		
//		if (w1.length() != w2.length())
//			return false; // not necessary since all words have same length.  Call me paranoid
		...
		for (int i = 0; i< w1.length(); i++) {
			if (w1.charAt(i) != w2.charAt(i)) {
				diffCount++;
				if (diffCount >1)
					return false; // if more than 1 character different, then not neighbors 
			}
		}
		return true;
