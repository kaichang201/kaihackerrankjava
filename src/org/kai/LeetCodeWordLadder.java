...
public static void main(String[] args) {
	...
	LeetCodeWordLadder me = new LeetCodeWordLadder();
	...
	System.out.println(me.ladderLength(beginWord1, endWord1, Arrays.asList(wordList1)));
	System.out.println(me.ladderLength(beginWord2, endWord2, Arrays.asList(wordList2)));
	System.out.println(me.ladderLength(beginWord3, endWord3, Arrays.asList(wordList3)));
	System.out.println(me.ladderLength(beginWord4, endWord4, Arrays.asList(wordList4)));
	System.out.println(me.ladderLength(beginWord5, endWord5, Arrays.asList(wordList5)));
	
	System.out.println("Time taken " + (System.currentTimeMillis() - startTime));
	System.out.println(me.ladderLength(beginWord6, endWord6, Arrays.asList(wordList6)));
	...
	public int ladderLength(String beginWord, String endWord, List<String> wordList) {
		...
		System.out.println("  Created map " + (System.currentTimeMillis() - startTime));
	...
	public int bfs(String beginWord, String endWord, Map<String, Set<String>> m) {
		...
		while (!q.isEmpty()) {
			...
			s.removeAll(visited);
			childCount+=s.size(); // increment by the number of children I am queing up
			q.addAll(s);
		}
...
//			return false; // not necessary since all words have same length.  Call me paranoid
	...
	// DFS with backtracking
	public int dfs(String beginWord, String endWord, Map<String, Set<String>> m, Set<String> visited) {		
		int minSize = 0;
		...
		// clone a set.  remove all visited neighbors.
		Set<String> s = new HashSet<>(m.get(endWord));
		s.removeAll(visited);
		
		// if noone left to visit, failure end condition
		if (s.size() == 0)
			return 0;
		
		for (String w: s) {  // iterate through all neighbors not yet visited.
			visited.add(w);
			int result = dfs(beginWord, w, m, visited); // walk from endWord to beginWord
			visited.remove(w); // backtracking

			if (result > 0  && (minSize == 0 || result <minSize))
				minSize = result;  // found new minimum
		}
		return minSize;
